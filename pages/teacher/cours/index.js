...
class Cours extends React.Component {
  ...
  handleSubmit = (event) => {
    ...
    const data = {
      ...
      program: parseInt(this.state.titre),
    };
    if (this.state.new == 0) {
      axios
      ...
      axios
        .put(`api/school/lecon/${this.state.titre}`, data)
  ...
  componentDidUpdate(prevProps, prevState) {
    if (this.state.program !== prevStae.program) {
      try {
        const contentCours = axios.get(`api/school/lecon/${this.state.titre}`);
        const cours = contentCours.data.content;
        this.setState({ content: cours, new: 1 });
      } catch (err) {
        console.log(err);
      }
    }
      ...
      <LayoutT title="Cours">
        <div className="panneauStyle">
          <div className="panneauClasse">
            <SiGoogleclassroom size="20px" /> <span>{this.state.classe}</span>
          </div>
          <div id="triangle"></div>
...
export async function getServerSideProps() {
  try {
    const user = axios.get(`api/user/curentuser`);
